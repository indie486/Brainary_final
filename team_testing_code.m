...
function [sub_outcome_binary, sub_out_probability, cpc] = team_testing_code(model,input_directory,patient_id,verbose)
...
sub_good_model=model.sub_good_model;
sub_poor_model=model.sub_poor_model;
...
try
    ...
    % ------------ sub model ------------ %
    if outcome_binary==0
        sub_p_all=ones(f_m_size,time_m_size)*-1;
        for m_i=1:time_m_size
            if m_i==1
                sub_features=t24_tmp;
            elseif m_i==2
                sub_features=[t24_tmp;t48_tmp];
            elseif m_i==3
                sub_features=[t48_tmp;t72_tmp];
            end

            if sum(sum(sub_features))~=0
                if size(sub_features,1)>0
                    decision_all=zeros(size(sub_features,1),f_m_size);
                    for mf_i=1:f_m_size
                        model_solo=sub_good_model{m_i,mf_i};
                        decision_all(:,mf_i)= predict(model_solo,sub_features(:,fm_range{mf_i}));
                        clear model_solo
                    end
                    % decision_all
                    sub_p_all(:,m_i)=mean(decision_all);
                end
                clear decision_all
            end
            clear sub_features mean_tmp
        end
        ...
        for m_i=1:time_m_size
            ...
            if sum(sum(sub_features))~=0
                if size(sub_features,1)>0
                    decision_all=zeros(size(sub_features,1),f_m_size);
                    for mf_i=1:f_m_size
                        model_solo=sub_poor_model{m_i,mf_i};
                        decision_all(:,mf_i)= predict(model_solo,sub_features(:,fm_range{mf_i}));
                        clear model_solo
                    end
                    % decision_all
    ...
    % method별 weight
    sub_w_all=sub_p_all;
    sub_w_all(1,:)=sub_w_all(1,:)*0.24;
    sub_w_all(2,:)=sub_w_all(2,:)*0.25;
    sub_w_all(3,:)=sub_w_all(3,:)*0.25;
    sub_w_all(4,:)=sub_w_all(4,:)*0.26;
    sub_outcome_all=sum(sub_w_all);

    % 시간대별 weight
    avail_m=find(sub_outcome_all(1,:) >= 0);
    if length(avail_m)==3
        w_p=[0.3 0.4 0.3];
    elseif length(avail_m)==2
        if length(intersect(avail_m,[1 2]))==2
    ...
    sub_out_probability=sub_outcome_all*w_p';
    % current_outcome
    if sub_out_probability <= prob_threshold
        sub_outcome_binary=0;
    else
        sub_outcome_binary=1;
    end
    % cpc
    if sub_out_probability <= 0.5
        if sub_out_probability <= 0.3
            cpc=1;
        else
            cpc=2;
        end
    else
        if sub_out_probability <= 0.6
            cpc=3;
        elseif sub_out_probability <= 0.7
            cpc=4;
        else
            cpc=5;
        end
    ...
    sub_out_probability=1; % poor
    sub_outcome_binary=1; % poor
    cpc=5; % poor
end
...
% prob. modify
if sub_out_probability==0
    sub_out_probability=mod(randn(1,1),1)/100;
elseif sub_out_probability==1
    sub_out_probability=1-mod(randn(1,1),1)/100;
end
